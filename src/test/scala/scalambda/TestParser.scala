...
class LambdaCalcParserSpec extends FlatSpec with Matchers {
  ...
  it should "parse complex expressions" in {
    p("/x./y. (x y)") should be(
      LLam("x", LLam("y", LApp(LVar("x"), LVar("y")))))
    p("/x.(/y.(x y) (/z.z) x)") should be(
      LLam("x",
           LApp(LApp(LLam("y", LApp(LVar("x"), LVar("y"))),
                     LLam("z", LVar("z"))),
                LVar("x")))
    )
    p("/x./y.(x y (/z.z x))") should be(
      LLam("x",
           LLam("y",
                LApp(LApp(LVar("x"), LVar("y")),
                     LApp(LLam("z", LVar("z")), LVar("x")))))
    )
...
class LibParserSpec extends FlatSpec with Matchers {
  ...
  "A LibParser" should "parse definitions from files" in {
    parser.parseFile(testFile) should be(
      Map[String, LExp](
